...
import {
  STRATEGY_FIELD_GROUPS,
  formatStrategyFactor as f2,
  formatStrategyNumber as fmt,
...
<template>
  <section class="strategy-profile-inputs">
    ...
    <details class="spi-param-details" :open="input.strategyProfile === 'custom'">
      <summary>
        <span>参数调节</span>
        <strong>{{ input.strategyProfile === 'custom' ? '模拟档位可编辑' : '模拟档位只读' }}</strong>
      </summary>
      <div v-if="input.strategyProfile === 'custom'" class="spi-groups">
        <section v-for="group in STRATEGY_FIELD_GROUPS" :key="group.title">
          <h4>{{ group.title }}</h4>
          <div class="spi-grid">
      ...
      <p v-else>这些档位参数只负责过滤、情景仓位和冷却；不构成实盘仓位建议。</p>
    </details>
...
.spi-head {
  display: grid;
  grid-template-columns: 1fr minmax(100px, 0.7fr);
  gap: 8px;
  align-items: center;
}
.spi-head strong {
...
.spi-step dl {
  display: grid;
  grid-template-columns: minmax(62px, 0.42fr) 1fr;
  gap: 3px 7px;
  margin: 0;
  font-size: 0.66rem;
}
...
.spi-calibration span {
  color: var(--green);
  font-size: 0.64rem;
  font-weight: 900;
}
.spi-calibration strong {
  color: var(--ink);
  font-size: 0.72rem;
  font-variant-numeric: tabular-nums;
}
.spi-calibration small {
  grid-column: 1 / -1;
  color: var(--muted);
  font-size: 0.62rem;
  line-height: 1.35;
...
.spi-param-details summary strong {
  color: var(--muted);
  font-size: 0.62rem;
}
.spi-groups {
...
.spi-links header span {
  color: var(--green);
  font-size: 0.62rem;
  font-weight: 900;
}
.spi-links header strong {
  color: var(--muted);
  font-size: 0.62rem;
  text-align: right;
}
.spi-links dl {
  display: grid;
  grid-template-columns: minmax(74px, 0.55fr) 1fr;
  gap: 4px 8px;
  margin: 0;
  font-size: 0.66rem;
}
...
.spi-modules article {
  display: grid;
  grid-template-columns: minmax(58px, 0.55fr) minmax(42px, auto) 1fr;
  gap: 6px;
  align-items: baseline;
  padding: 5px 7px;
  border: 1px solid var(--line);
...
.spi-modules article.research span {
  color: var(--muted);
}
.spi-modules small {
  color: var(--muted);
