...
const MAX_BAR = 170
const viewportTop = computed(() => Math.max(0, Math.round(props.viewport?.top ?? 0)))
...
function priceY(price, range) {
  const mapped = props.viewport?.priceToY?.(price)
  if (Number.isFinite(mapped)) return Math.max(0, Math.min(viewportHeight.value, mapped))
    ...
    <svg :viewBox="`0 0 ${W} ${viewportHeight}`" class="scp-svg">
      <g class="scp-bars">
        ...
        >
          ...
          <rect :x="bar.x" :y="bar.y" :width="bar.width" :height="bar.height" rx="1.5" class="scp-total" />
          <rect
      ...
      <text v-if="currentY !== null" x="18" :y="currentY + 12" class="scp-label current">
        现价 {{ money(profile.currentPrice) }}
      </text>
...
.scp {
  position: absolute;
  top: v-bind('`${viewportTop}px`');
  right: 54px;
  z-index: 14;
  width: min(260px, 26%);
  height: v-bind('`${viewportHeight}px`');
  pointer-events: none;
  overflow: hidden;
}
.scp-svg {
...
.scp-total {
  fill: rgba(120, 120, 120, 0.11);
}
.scp-up {
  fill: rgba(14, 117, 88, 0.34);
}
.scp-down {
  fill: rgba(169, 50, 38, 0.24);
}
.scp-bars .value .scp-total {
  fill: rgba(14, 117, 88, 0.13);
}
.scp-bars .poc .scp-total {
  fill: rgba(139, 90, 22, 0.32);
}
.scp-bars .current .scp-total {
  stroke: rgba(39, 79, 159, 0.7);
  stroke-width: 1;
}
.scp-poc-line {
  stroke: rgba(139, 90, 22, 0.82);
  stroke-width: 1.2;
  stroke-dasharray: 3 2;
}
.scp-current-line {
  stroke: rgba(39, 79, 159, 0.82);
  stroke-width: 1.2;
}
.scp-label,
.scp-watermark {
