...
const ivP = computed({
  get: () => pctOut(props.input.iv),
  set: (v) => {
    props.input.iv = pctIn(v)
    props.input.ivSource = 'manual-scenario'
  },
})
...
const exitTargetP = computed({
  get: () => pctOut(props.input.exitTargetReturn),
  set: (v) => {
    props.input.exitTargetReturn = pctIn(v)
  },
})
