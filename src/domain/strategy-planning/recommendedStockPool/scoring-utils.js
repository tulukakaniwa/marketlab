...
// 文案格式化（formatHit / formatPct）以及 z 正态参考极端度换算。
...
export function formatHit(id, m) {
  switch (id) {
    case 'lpValuePercentile':
      return `合成几何 P${(m.lpValuePercentile * 100).toFixed(1)}%`
    case 'zScore':
      return `z=${m.zScore.toFixed(2)}σ`
    case 'lpZone':
      return '合成区间下侧'
    case 'costSlope':
      return '成本锚↑'
...
export function deviationReferenceFromZ(z) {
  if (!Number.isFinite(z)) return null
  const abs = Math.abs(z)
  const cdf = standardNormalCdf(abs)
  const deviationPercentile = Math.max(0, Math.min(1, 2 * cdf - 1))
  return {
    deviationPercentile,
    twoSidedTailProbability: 1 - deviationPercentile,
    semantics: 'normal-reference-extremeness-not-reversion-probability',
  }
}

export function deviationPercentileFromZ(z) {
  return deviationReferenceFromZ(z)?.deviationPercentile ?? null
}
