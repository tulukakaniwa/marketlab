...
export function summarizeRegime(payload) {
  ...
  if (costDistance < -0.05) return `深度折价 ${distance}，${baseline}下方较远，适合分批吸纳`
  if (costDistance < -0.01) return `小幅折价 ${distance}，低于${baseline}，可再等更便宜或开始买`
  if (costDistance > 0.05) return `溢价 ${distance}，高于${baseline}较多，仅做仓位保护`
...
export function summarizeReason(payload) {
  ...
  if (side === 'sell') {
    if (costDistance > 0.05) return `溢价 ${distance}，已偏离均价较多，只用底仓减压`
    return `溢价 ${distance}，仅做仓位保护，不主动加仓`
  }

  if (side === 'buy') {
    if (costDistance < -0.05) return `折价 ${distance}，价格便宜了不少，可分批吸筹`
    if (costDistance < -0.01) return `小幅折价 ${distance}，再等更便宜一点或轻仓试探`
