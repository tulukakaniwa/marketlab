...
- 旧 `targetReturn` 不再进入新 graph/order/profile 契约；边界迁移必须显式报 deprecated，不能静默兜底。
- `estimateCumulativeFundingProxy()` 分开输出 `basisFraction` 与 `cumulativeFundingProxy`；带符号金额另叫 `fundingCashflowQuote`，并由 `fundingCashflowSource` 区分已观测结算与显式情景。
- `netCarry()` 只消费同周期 `cumulativeFundingProxy`，不再二次乘期限。
