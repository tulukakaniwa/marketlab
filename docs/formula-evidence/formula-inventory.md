...
| `capitalEfficiency`, `capitalEfficiencyFrontier`, `impermanentLoss`, `lpResearchAttribution` | geometry plus same-horizon LP attribution                         | research-only |
...
| `deviationScore`         | `z`, absolute-deviation percentile and two-sided tail mass; no future-reversion probability     | descriptive reference              |
| `lpResearchAttribution`  | keeps CE geometry separate; adds only same-notional, same-horizon IL, path fee return and costs | research-only                      |
...
| `compareFeeCarryToTheta` | basis-gated economic analogy; never an identity                                                 | research-only                      |
...
| `signalStrength`                  | `1 - 2\*(1-Phi(                                                                  | z                                   | ))` normal-reference extremeness from z-score | implemented; not confidence/win probability |
