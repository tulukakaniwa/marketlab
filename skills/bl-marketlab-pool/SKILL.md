...
# bl-marketlab-pool — Market Lab 推荐股票池拉取 & 企业微信文案（v2 五维评分）
...
**分级阈值**：
- **focus（重点关注）**：buyScore ≥ 65（至少 2 个维度拉满）
- **wait（等待）**：40 ~ 64（常见瓶颈是成本锚还在↓）
- 不入选：< 40（不出现在 items 里）
...
3. **生成文案**（默认 `tier=focus` + `format=narrative`）：
   ...
   - `tier=both` 时在 focus 之后再加一段 `▎等待（40~64 分）` 列表，但只列 `label / symbol / buyScore / 简短一句话`，不必把 narrative 全部展开
